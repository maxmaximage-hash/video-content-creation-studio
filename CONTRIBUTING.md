...
- `main` 只接受已经完成本地安装和真实应用验收的稳定版本。
- 下一版本在 `codex/vX.Y.Z-workbench` 开发；同一轮的小修复可以合并为少量有意义的提交，不要求一个 bug 一个提交。
- 开发期使用 `X.Y.Z-dev.N` 版本号，正式发布前再改为稳定的 `X.Y.Z`。
- 功能自动化通过只代表防回退。抖音、小红书采集是否成功，必须在真实安装版中使用真实链接单独记录。
- 详细发布门禁见 [docs/release-workflow.md](docs/release-workflow.md)。
