...
Video Hub 通过 Eagle 本机 API 和项目内固定文件夹 ID 管理媒体。换电脑后无需修改 Video Hub 配置，但必须满足：

- Eagle 已启动，本机 API 使用项目约定端口 `41595`。
- 同一团队 Eagle 资料库已挂载到代码约定的团队路径。
- Eagle 资料库保留项目既有文件夹结构和文件夹 ID。

这些固定路由随源码发布；`.library` 只保存 Eagle item 关联，不复制 Eagle 原文件。
